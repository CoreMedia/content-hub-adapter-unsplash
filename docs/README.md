...
Go to [Release](https://github.com/CoreMedia/content-hub-adapter-unsplash/releases) and download the version that matches your CMCC release version.
The ZIP file provides the Maven workspace of the plugin.

## Activate the Plugin

The unsplash contenthub adapter is a plugin for studio-server and studio-client.
...
2. Build the `content-hub-adapter-unsplash`
   ...
   2. Run `pnpm install && pnpm -r run build && pnpm -r run package` in the folder `studio-client`.
      ...
      Checkpoint: A zip file exists in `studio-client/apps/main/content-hub-adapter-unsplash/build`.
3. Create a directory for studio-server plugins, e.g. `/tmp/studio-server-plugins`,
   and copy `content-hub-adapter-unsplash/studio-server/target/studio-server.content-hub-adapter-unsplash-<version>.zip`
   into that directory.
...
5. Start the studio client with an additional property `-DadditionalPackagesDirs=/.../content-hub-adapter-unsplash/studio-client/target/app`
